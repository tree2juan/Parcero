...
const UI_STRINGS = {
  en: {
    ...
    "library.sourceNote.after": "; the level shown comes from it. Register and regionality are published as reviewed and approved for the list as a whole — they read the same on every verb, so treat them as a general guide rather than a judgement on each one, and the Report an error tab is where corrections start.",
  ...
  es: {
    ...
    "library.sourceNote.after": "; de ahí sale el nivel que ves. El registro y la regionalidad se publican como revisados y aprobados para toda la lista — son iguales en cada verbo, así que tómalos como una guía general y no como un juicio sobre cada uno; la pestaña Reportar un error es por donde empiezan las correcciones.",
