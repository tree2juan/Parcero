...
const TIER_A = [
  ...
  /*
   * The families above were incomplete: "favourite" was mapped, "favour" was
   * not, and the guard had the same hole, so 125 instances of "favour"
   * survived the migration and its check. Longer inflections must come first
   * — mapping "favour" before "favourable" turns it into "favorable" only by
   * luck of the suffix, and "instalment" before "instalments" would leave
   * "installments" as "installmentss".
   *
   * Nothing here collides with a Spanish word. "cheque" and "calibre" are
   * deliberately absent from both this list and the guard for that reason.
   */
